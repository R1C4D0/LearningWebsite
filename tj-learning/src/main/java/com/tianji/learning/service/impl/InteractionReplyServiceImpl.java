...
//        获取问题PO
        ...
        if (isAnswer) {
            // 是回答，则需要更新问题的最新回答和回答数量
            interactionQuestion.setLatestAnswerId(reply.getId());
            interactionQuestion.setAnswerTimes(interactionQuestion.getAnswerTimes() + 1);
        } else {
            // 是评论，则需要更新问题的评论数量
            InteractionReply answerId = this.getById(replyDTO.getAnswerId());//评论回答的id
            answerId.setReplyTimes(answerId.getReplyTimes() + 1);
            this.updateById(answerId);
        }
//        4.判断是否是学生回答，如果是学生回答，则需要更新问题的状态为未审核
    ...
    public PageDTO<ReplyVO> queryReplyVOPage(ReplyPageQuery query) {
        ...
        if(targetReplyIds.size() > 0) {
            ...
            Set<Long> targetUserIds = targetReplies.stream()
                    .filter(Predicate.not(InteractionReply::getAnonymity).or(r -> true))
